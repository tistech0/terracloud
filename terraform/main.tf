...
# Update other NSGs to allow traffic from Bastion
resource "azurerm_network_security_rule" "allow_bastion_inbound" {
  ...
  network_security_group_name = element(
    [
      azurerm_network_security_group.frontend_nsg.name,
      azurerm_network_security_group.backend_nsg.name,
      azurerm_network_security_group.database_nsg.name
    ],
    count.index
  )
}